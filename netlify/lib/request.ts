...
export const parseOrderInput = (body: unknown, paymentMethod: PaymentMethod): OrderInput | null => {
  ...
  const receiptLegalName = typeof data.receiptLegalName === 'string' ? data.receiptLegalName.trim() : ''
  const checkoutId = typeof data.checkoutId === 'string' ? data.checkoutId.trim() : ''
  const promotionCode = typeof data.promotionCode === 'string' ? data.promotionCode.trim().toUpperCase().replace(/\s+/g, '') : ''
  ...
  if (receiptType === 'factura' && (!receiptLegalName || receiptLegalName.length > 200)) return null
  return { customer, phone, email, address, fulfillment, receiptType, checkoutId, ...(receiptType === 'boleta' && dni ? { dni } : {}), ...(receiptType === 'factura' ? { ruc, receiptLegalName } : {}), ...(promotionCode ? { promotionCode } : {}), items: items.items, paymentMethod }
}
