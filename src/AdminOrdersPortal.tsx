...
type AdminOrder = {
  ...
  receipt_type: 'boleta' | 'factura' | null
...
const orderFields = 'id, order_number, customer_name, customer_email, customer_phone, order_type, delivery_address, delivery_reference, payment_method, payment_status, gift_card_amount, other_payment_amount, other_payment_method, receipt_type, receipt_document_number, receipt_legal_name, status, total, notes, created_at, rejection_reason, rejection_comment, rejected_at, rejected_by'
...
const paymentLabel = (method: string) => ({ cash: 'Efectivo', card: 'Tarjeta', wallet: 'Yape/Plin', gift_card: 'Gift Card', gift_card_culqi: 'Gift Card + Culqi' }[method] ?? method)
const paymentStatusLabel = (status: string) => ({ paid: 'Pagado', pending: 'Pendiente', refund_pending: 'Reembolso Culqi pendiente', refunded: 'Gift Card restituida', failed: 'Fallido', expired: 'Vencido' }[status] ?? status)
