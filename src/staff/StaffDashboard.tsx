...
export function StaffDashboard({ profile, userId, onSignOut }: { profile: StaffProfile; userId: string; onSignOut: () => Promise<void> }) {
  ...
  const kitchenStateLabel = kitchenStatus?.value.manual_closed ? 'Cocina cerrada manualmente' : kitchenStatus?.value.force_open ? 'Cocina abierta manualmente' : scheduleOpen ? 'Cocina abierta por horario' : 'Cocina cerrada por horario'
  const isKitchenOpen = !kitchenStatus?.value.manual_closed && (kitchenStatus?.value.force_open === true || scheduleOpen)
  ...
  const updateKitchenStatus = async (manualClosed: boolean, forceOpen = !manualClosed) => {
    if (!canManageKitchen) return
    setSavingKitchenStatus(true); setMessage('')
    const { data, error } = await supabase.from('app_settings').update({
      value: { manual_closed: manualClosed, force_open: forceOpen, reason: manualClosed ? kitchenClosureReason || null : null },
      updated_by: userId,
    ...
    if (error || !data) setMessage('No se pudo actualizar el estado de cocina. Intenta nuevamente.')
      ...
      setMessage(manualClosed ? 'Cocina cerrada temporalmente.' : forceOpen ? 'Cocina abierta manualmente para pedidos web.' : 'Cocina en horario automático.')
    }
  ...
  const createMember = async (event: FormEvent) => {
    ...
    if (!newName.trim() || !validDocument || !cleanPhone || !cleanEmail) { setMessage(newDocumentType === 'DNI' ? 'Completa nombre, DNI de 8 dígitos, teléfono y correo electrónico.' : 'Completa nombre, Carné de Extranjería de 9 a 11 dígitos, teléfono y correo electrónico.'); return }; if (!validEmail(cleanEmail)) { setMessage('Ingresa un correo electrónico válido.'); return }
    setSavingMember(true); setMessage('')
    try { const [{ data: documentMatch }, { data: phoneMatch }] = await Promise.all([supabase.from('members').select('id').eq('document_type', newDocumentType).eq('document_number', cleanDocumentNumber).maybeSingle(), supabase.from('members').select('id').eq('phone', cleanPhone).maybeSingle()]); if (documentMatch) { setMessage('Ya existe un socio con ese tipo y número de documento.'); return }; if (phoneMatch) { setMessage('Ya existe un socio con ese teléfono.'); return }; const { data, error } = await supabase.from('members').insert({ full_name: newName.trim(), document_type: newDocumentType, document_number: cleanDocumentNumber, phone: cleanPhone, email: cleanEmail, birth_date: newBirthDate || null, marketing_consent: marketingConsent, status: 'active' }).select(memberFields).single<Member>(); if (error || !data) { setMessage(error?.code === '23505' ? 'Ya existe un socio con ese documento o teléfono.' : 'No se pudo crear el socio.'); return }; const { data: sessionData } = await supabase.auth.getSession(); const welcomeResponse = sessionData.session ? await fetch('/.netlify/functions/send-member-welcome', { method: 'POST', headers: { Authorization: `Bearer ${sessionData.session.access_token}`, 'content-type': 'application/json' }, body: JSON.stringify({ memberId: data.id }) }) : null; await loadMemberDetails(data.id); setDocumentNumber(cleanDocumentNumber); setNewName(''); setNewDocumentType('DNI'); setNewDocumentNumber(''); setNewPhone(''); setNewEmail(''); setNewBirthDate(''); setMarketingConsent(false); setShowCreate(false); setMessage(welcomeResponse?.ok ? 'Socio creado y código de bienvenida enviado por email.' : 'Socio creado. No se pudo enviar el código por email; inténtalo nuevamente desde administración.') } catch { setMessage('El socio se creó, pero no se pudo cargar su información. Búscalo por documento.') } finally { setSavingMember(false) }
  }
  ...
  return <main className="staff-page"><section className="staff-card staff-dashboard"><header className="staff-header"><div><p className="eyebrow">THE BLACK CAT · STAFF</p><h1>Hola, {profile.display_name}</h1><p className="staff-role">Rol detectado: {profile.role}</p></div><button className="back-button staff-signout" type="button" onClick={() => void onSignOut()}>Cerrar sesión</button></header>
    {canUseBasicFeatures && <GiftCardsPanel profile={profile} />}
    {canUseBasicFeatures && <WhatsAppAlerts />}
