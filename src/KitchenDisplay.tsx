...
import { ProductAvailabilityPanel } from './ProductAvailabilityPanel'
...
type KitchenOrder = { id: string; order_number: string; customer_name: string; customer_phone: string; order_type: 'delivery' | 'pick_up'; delivery_address: string | null; delivery_reference: string | null; payment_status: string; receipt_type: 'boleta' | 'factura' | null; status: KitchenStatus; total: number; notes: string | null; created_at: string; order_items: KitchenItem[] }
...
export function KitchenDisplay() {
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      const { data, error } = await supabase.from('orders').select('id, order_number, customer_name, customer_phone, order_type, delivery_address, delivery_reference, payment_status, receipt_type, status, total, notes, created_at, order_items(id, product_name, category, quantity, unit_price, notes)').in('status', ['nuevo', 'preparando', 'listo', 'en_camino']).or('payment_method.eq.cash,payment_status.eq.paid').order('created_at', { ascending: true })
  ...
  const isKitchenOpen = !kitchenAvailability.manual_closed && (kitchenAvailability.force_open || scheduleOpen)
