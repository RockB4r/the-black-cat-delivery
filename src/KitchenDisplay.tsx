...
export function KitchenDisplay() {
  ...
  const authorize = async (nextUser: User | null) => {
    if (!nextUser) { setUser(null); setIsAdmin(false); setLoading(false); return false }
    if (nextUser.email?.toLowerCase() === KITCHEN_EMAIL) { setUser(nextUser); setIsAdmin(false); setLoading(false); return true }
    const { data: profile } = await supabase.from('staff_profiles').select('role, active').eq('user_id', nextUser.id).maybeSingle()
    if (profile?.active && profile.role === 'admin') { setUser(nextUser); setIsAdmin(true); setLoading(false); return true }
